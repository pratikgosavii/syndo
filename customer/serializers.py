...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for item in items_data:
            ...
            # Print-type rule: if computed line total is less than sales price, use the sales price.
            ...
            if sales_price_item is not None and base_total < sales_price_item:
                base_total = sales_price_item
                _req_log = logging.getLogger("request")
                _msg = (
                    f"[order_api create] product={getattr(product1, 'name', '')} base_total={base_total} < "
                    f"sales_price_item={sales_price_item} → line_total=sales_price+addons={base_total + addon_total} (addon_total={addon_total})"
                )
                print(_msg)
                _req_log.info(_msg)
            elif sales_price_item is not None and base_total >= sales_price_item and is_print_product:
                # Keep the computed print-job total when it is already above the floor.
                _req_log = logging.getLogger("request")
                _msg = (
                    f"[order_api create] product={getattr(product1, 'name', '')} base_total={base_total} >= "
            ...
            if is_print_product and item_print_job:
                line_total = base_total + addon_total
            else:
