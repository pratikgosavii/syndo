...
class CustomerOrderViewSet(viewsets.ModelViewSet):
    ...
    def check_delivery_availability(self, request):
        ...
        # TEMPORARY: Always return success for Cashfree testing
        from vendor.models import DeliveryMode
        delivery_mode = DeliveryMode.objects.filter(user=vendor_user).first() if vendor_user else None
        logger.info("[check_delivery_availability] Delivery restriction bypassed for Cashfree testing. Delivery type: %s", delivery_type)
        logger.info("=" * 80)
        return Response({
            "ok": True,
